...
export const cartSlice = createSlice({
  ...
  reducers: {
        ...
        state.cart_products.map((item) => {
          if (item._id === payload._id) {
            if (item.quantity >= item.orderQuantity + state.orderQuantity) {
              ...
              notifySuccess(`${state.orderQuantity}개 ${item.title}가 장바구니에 추가되었습니다`);
            } else {
              notifyError("이 상품의 재고가 부족합니다!");
              state.orderQuantity = 1;
            }
          }
          return { ...item };
    ...
    decrement: (state, { payload }) => {
      state.orderQuantity =
        state.orderQuantity > 1 ? state.orderQuantity - 1 : (state.orderQuantity = 1);
    },
    ...
    clearCart: (state) => {
      const isClearCart = window.confirm("모든 상품을 삭제하시겠습니까?");
      if (isClearCart) {
        state.cart_products = [];
      }
      setLocalStorage("cart_products", state.cart_products);
